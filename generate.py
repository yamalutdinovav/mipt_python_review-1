...
parse.add_argument('--model',
                   help='Обязательный аргумент. Путь к файлу, из которого загружается модель.',
                   required=True)
parse.add_argument('--seed', help='Начальное слово. Если не указано, выбираем слово случайно',
                   action='store', dest='seed')
parse.add_argument('--length', help='Обязательный аргумент. Длина генерируемой последовательности',
                   required=True, action='store', dest='length')
parse.add_argument('--output',
                   help='Файл, в который будет записан результат. Если не указан, текст выводится в stdout')
...
def weighted_choice(elements):
    ...
    :type elements: dict
    :return: Возвращает слово, случайно выбранное из словаря с учетом частоты вхождения
    :rtype: str

    '''
    total = sum(weight for token, weight in elements.items())  # Cуммарная частота вхождения слов
...
def write_phrase(phrase):
    ...
    :return: Записывает фразу в указанный файл ( или выводит в sys.stdin, если не указан аргумент --output)
